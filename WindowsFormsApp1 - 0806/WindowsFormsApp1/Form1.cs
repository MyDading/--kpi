...
namespace WindowsFormsApp1
{
    public partial class Form1 : CCSkinMain
    {
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            ...
            //cmd.CommandText = "SELECT * from kpi_msg WHERE 姓名='呂運鼎'";
            cmd.CommandText = "SELECT * from kpi_msg WHERE 姓名='" + 登录用户 + "'";   //数据库数据建好后启用该句
            ...
            {
                MessageBox.Show(登录用户);
            }

            conn.Close();
            //this.CxFlatRoundButton16_Click(null, null);
            */
            ///*******功能结束********///
...
            con.Open();//打开连接
            ...
            //     {
                    ...
                    string insert = "insert ignore into kpi_msg(kpi,kpi辅项,kpi细项,审核方式,评核计算方式,目标值,比重,达成数,目标数,项次,项目,系统编号,细项,工号) values " +
                    "( '专案质量', '进度管理','" + kpi细项 + "'," + "'" + 审核方式 + "'," + "'" + 评核计算方式 + "'," + "'" + 目标值 + "'," + "'" + 比重 + "'," + "'" + 达成数 + "'," + "'" + 目标数 + "'," + "'" + 项次 + "'," + "'" + 项目 + "'," + "'" + 系统编号 + "'," + "'" + 细项 + "'," + "'" + 工号 + "')";
        ...
        private void Kpi信息刷新_Click(object sender, EventArgs e)
        {
            ...
            cmd.CommandText = "SELECT * from kpi_msg WHERE 姓名='" + 登录用户 + "' LIMIT 3,1";   //数据库数据建好后启用该句
